...
**План реализации:** [plan-frontend.md](../plans/plan-frontend.md)
...
`dataProvider` — статический, в scaffold-шаблоне. Один универсальный файл, маппит стандартный CRUD на REST-конвенции (`GET /resource`, `POST /resource`, `GET /resource/:id` и т.д.). Ресурсы задаются конфигом при инициализации Refine. Генератор не нужен — `RouterStubGenerator` обеспечивает стандартные конвенции, `dataProvider` на них опирается. `authProvider` подключается к нашему auth-слою — единый JWT, единая сессия с юзерскими страницами.
...
Генерация TypeScript-интерфейсов из OpenAPI-схемы, которую FastAPI генерит автоматически из зарегистрированных роутеров и Pydantic-схем.

Два варианта извлечения OpenAPI:

**Вариант A: Поднятый бэкенд.**
```
...
**Вариант Б: Статический дамп (выбран).**
```
scripts/dump_openapi.py → from app.main import app; app.openapi()
    → openapi.json (без БД, без брокеров, без сети)
    → openapi-typescript → frontend/src/generated/types.ts
```

**Почему статический дамп, а не поднятый бэкенд:**
- **CI и pre-push без инфраструктуры** — не нужно поднимать БД, Redis, весь Docker Compose для проверки типов. Быстрее на порядок.
- **Бесплатная проверка качества кода** — если кто-то написал сайд-эффект на уровне импорта (сетевой вызов, инициализация БД), дамп упадёт. Это заставляет держать модули чистыми — полезный архитектурный инвариант.
- Swagger UI (`/docs`) по-прежнему доступен при `make dev-start` — документация никуда не девается.

`openapi.json` коммитится в репозиторий — фронтенд-разработчики без Python берут типы оттуда.
...
### 6. SchemasGenerator — без изменений

`SchemasGenerator` остаётся как есть: генерирует и ORM-модели, и Pydantic-варианты (Create/Read/Update) в один файл `shared/shared/generated/schemas.py` через `datamodel-code-generator`. Всё перегенерируется при `make generate`.

**Отвергнуто: переименование в ModelsGenerator + отдельный SchemaStubGenerator.**
Рассматривалось разнесение: ORM-модели отдельно (regenerated), Pydantic-схемы отдельно (write-once, per-service). Решение — не делать:
- **YAGNI** — реальной потребности кастомизировать сгенерированные схемы пока нет. Когда появится — можно добавить `SchemaStubGenerator` тогда.
- **Нулевой breaking change** — текущие импорты из `shared.shared.generated.schemas` продолжают работать.
- **Фокус Фазы 1** — ценность в `RouterStubGenerator`, а не в рефакторинге генерации схем.

Если разработчику нужна кастомная схема — он пишет её руками рядом и меняет импорт в роутере (роутер write-once, можно редактировать).

### 7. Write-once стабы для роутеров

По аналогии с `ControllersGenerator` (генерирует файл только если его нет, потом не трогает), добавить новый генератор:
...
- Импортирует Pydantic-схемы из `shared.shared.generated.schemas`
...
Две dependency-функции (`require_authenticated_user`, `require_admin`) пишутся один раз в шаблоне проекта. `public` и `custom` не требуют dependency.
...
models.yaml (сущности и их поля)
    └──→ ORM-модели + Pydantic-схемы (SchemasGenerator, перегенерируется)

доменные спеки services/*/spec/*.yaml (что можно делать с сущностями)
...
Phase 1: Backend стабы
         ...
         (SchemasGenerator без изменений)

Phase 2: OpenAPI + TS-типы
         - Статический дамп OpenAPI → openapi.json
         - openapi-typescript → TS-типы
         - Базовый apiClient в scaffold-шаблоне

Phase 3: UI/UX инфраструктура
         - Astro + React, shadcn/ui, ThemeContext, Error Boundary

Phase 4: Auth
         - AuthContext, ProtectedRoute, страница логина (3 роли)
         - Параметризация через Copier (frontend_auth флаг)

Phase 5: Refine интеграция
...
Статический `dataProvider` покрывает **только стандартный CRUD** по REST-конвенциям.
Для кастомных бизнес-действий (например: `approve_document`, `ban_user`) используется напрямую типизированный `apiClient` с вызовом endpoints, в обход `dataProvider`.
